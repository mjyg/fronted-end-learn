...
// 1. 调用了reconcileChildren
// 1. 初次，则创建Fiber子节点
// 2. 非初次， 更新Fiber节点，得到Fiber节点， 打上EffectTag标记
function updateHostRoot(current, workInProgress, renderExpirationTime) {
  pushHostRootContext(workInProgress);
  // 收集整合当前Fiber节点的必要状态属性
  ...
  // 处理updateQueue,设置workInProgress的memoizedState,expirationTime等属性
  processUpdateQueue(workInProgress, nextProps, null, renderExpirationTime);
  // 获取下级的reactElement对象, 用于生成Fiber子树
  ...
  if (nextChildren === prevChildren) {
    ...
    // 如果状态与以前相同，则是紧急情况，因为我们有目前没有任何工作到期。
    resetHydrationState();
    return bailoutOnAlreadyFinishedWork(
      current,
      workInProgress,
...
// 2. 根据不同的Fiber 节点类型，创建不同的Fiber节点, 构造 Fiber 树形结构
// 3. 给当前 Fiber 对象设置effectTag标记
function beginWork(
  current: Fiber | null,
  workInProgress: Fiber,
  renderExpirationTime: ExpirationTime,
  ...
  switch (workInProgress.tag) {
    ...
    case HostRoot:
      // updateHostRoot: return workInProgress.child, 改变了workInProgress指针的指向
